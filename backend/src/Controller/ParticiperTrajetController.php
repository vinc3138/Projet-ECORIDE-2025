...
class ParticiperTrajetController extends AbstractController
{
    ...
    public function participer(
        ...
        if (!in_array($privilegeId, [2, 3])) {
            return new JsonResponse(['error' => 'Accès interdit : privilèges insuffisants'], 403);
        }
        ...
        if (!$statutReservation) {
            throw $this->createNotFoundException('Statut non trouvé.');
        }
...
Votre réservation pour le trajet %s → %s est confirmée.
