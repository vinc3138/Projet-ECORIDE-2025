...
{/* ----------------------------------------------------------------------------------------------------------- */}
{/* Préparation pour intégration des durée & distance d'un trajet */}
{/* ----------------------------------------------------------------------------------------------------------- */} 
...
useEffect(() => {
  ...
  if (!token) return;
	...
	{/* Fetch Trajets */}
	{/* ----------------------------------------------------------------------------------------------------------- */} 
		fetch('https://127.0.0.1:8000/api/trajets/historique', {
		  headers: {
	...
	{/* Fetch Voiture */}
	{/* ----------------------------------------------------------------------------------------------------------- */} 
		fetch('https://127.0.0.1:8000/api/voiture_utilisateur', {
		  headers: {
			Authorization: `Bearer ${token}`,
			'Content-Type': 'application/json',
		  },
...
{/* ----------------------------------------------------------------------------------------------------------- */}
      if (token) {
        const userRes = await fetch('https://127.0.0.1:8000/api/user', {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
		...
		const fetchOtherData = async (role) => {
		  try {
			...
			if (role === 1 || role === 3) {
			  ...
			  // -----------------------------------------------------------------------------------------------------------
			  try {
				const avisRes = await fetch('https://127.0.0.1:8000/api/avis/chauffeur', {
				  headers: {
					Authorization: `Bearer ${token}`,
					'Content-Type': 'application/json',
				  },
			  ...
			  // -----------------------------------------------------------------------------------------------------------
			try {
					const preferencesRes = await fetch('https://127.0.0.1:8000/api/utilisateur/get_user_preference', {
					  method: 'GET',
					  headers: {
						'Authorization': `Bearer ${token}`,
						'Content-Type': 'application/json',
...
{/* ----------------------------------------------------------------------------------------------------------- */}
{/* Propositions des villes renseignées d'un nouveau trajet */}
{/* ----------------------------------------------------------------------------------------------------------- */}
...
{/* ----------------------------------------------------------------------------------------------------------- */} 
  const handleSubmit = e => {
    ...
    fetch('https://127.0.0.1:8000/api/create_trajet', {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${token}`,
        'Content-Type': 'application/json',
...
{/* ----------------------------------------------------------------------------------------------------------- */} 
  const handleVehiculeSubmit = e => {
    ...
    setError(null);
    // Validation simple
    if (!form.marque || !form.modele || !form.nb_place || !form.immatriculation || !form.couleur) {
      setError('Veuillez remplir tous les champs.');
      return;
    }

    fetch('https://127.0.0.1:8000/api/voiture/utilisateur', {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${token}`,
        'Content-Type': 'application/json',
...
{/* ----------------------------------------------------------------------------------------------------------- */} 
  const handleDeleteVehicle = id => {
    ...
    fetch(`https://127.0.0.1:8000/api/voiture/utilisateur${id}`, {
      method: 'DELETE',
			...
			<form onSubmit={handleSubmit}>
			  ...
			  {/* Ville d'arrivée */}
				<div className="mb-2">
				  ...
				  />
				  {errors.destination && (
					<div className="text-danger small">{errors.destination}</div>
				  )}
				</div>
			  ...
			  {/* Durée */}
			  <div className="mb-2">
				<label htmlFor="duree" className="form-label h6">Durée (h) <em>(renseigné automatiquement)</em></label>
				<input
			  ...
			  <div className="mb-3">
				...
				<MapComponent 
				  ...
				  departAddress={formData.adresse_depart} 
				  arriveeCity={formData.destination} 
				  arriveeAddress={formData.adresse_arrivee} 
				/>
		  ...
		  <div className="card-body">
			<form onSubmit={handleVehiculeSubmit}>
			  <div className="mb-2">
				...
				<input name="marque" className="form-control" value={newVehicule.marque} onChange={handleVehiculeSubmit} required />
			  </div>
			  <div className="mb-2">
				<label htmlFor="modele" className="form-label h6">Modèle</label>
				<input name="modele" className="form-control" value={newVehicule.modele} onChange={handleVehiculeSubmit} required />
			  </div>
			  <div className="mb-2">
				<label htmlFor="nb_places" className="form-label h6">Nombre de places</label>
				<input type="number" name="nb_places" className="form-control" value={newVehicule.nb_places} onChange={handleVehiculeSubmit} required />
			  </div>
			  <div className="mb-2">
				<label htmlFor="immatriculation" className="form-label h6">Immatriculation</label>
				<input name="immatriculation" className="form-control" value={newVehicule.immatriculation} onChange={handleVehiculeSubmit} required />
			  </div>
			  <div className="mb-2">
				<label htmlFor="couleur" className="form-label h6">Couleur</label>
				<input name="couleur" className="form-control" value={newVehicule.couleur} onChange={handleVehiculeSubmit} required />
			  </div>
