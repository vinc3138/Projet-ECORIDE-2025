...
export default function CompteUtilisateur() {
  ...
  const handlePhotoUpload = async (e) => {
    ...
    try {
      const response = await fetch('https://127.0.0.1:8000/api/upload_user_photo', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
  ...
  const handleSubmitInfos = async (e) => {
    ...
    try {
      const response = await fetch('/api/compte_utilisateur', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
  ...
  const handleAddCredits = async (e) => {
    ...
    try {
      const response = await fetch('https://127.0.0.1:8000/api/credits/add100', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const handleRoleChange = async () => {
    ...
    try {
      const response = await fetch('https://127.0.0.1:8000/api/changer-role', {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  if (loading) {
    // Affiche un message de chargement global si on veut (sinon on garde "Chargement..." dans user)
    return (
